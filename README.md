...
| [`github-cli`](github-cli/)                   | Baseline discipline for **every GitHub interaction**: use the `gh` CLI (not scraping or raw `api.github.com` curl), prefer `--json` for parsing, verify the remote before push, and gate every state-changing action (review, comment, merge, close, push) behind explicit user approval. |
